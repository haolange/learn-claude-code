...
```mermaid
flowchart TD
A["进入 agent_loop"] --> B["上下文压缩微压缩/自动压缩"]
B --> C["注入后台任务通知"]
C --> D["检查主导 inbox"]
D --> E["调用 LLM"]
...
## 性能考虑
- 上下文压缩策略：微压缩清理冗余 tool_result，超过阈值时自动压缩，避免超出模型上下文限制
...
## 结论
本仓库通过 20 个渐进式章节，系统性地展示了如何围绕“代理循环”构建一个可扩展、可组合的智能体编排系统。从单一 bash 工具到复杂的多代理协作与外部能力集成，每一步都保持核心循环不变，所有增强均以“机制”形式叠加。这种设计既保证了教学清晰度，也为生产级编排提供了坚实基础。
